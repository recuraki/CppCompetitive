...
int main() {
    ...
    vector<vector<vector<ll>>> dp(10200, vector<vector<ll>>(110, vector<ll>(110, 0LL)));
    dp.at(0).at(0).at(0) = 1LL;
    REP(i, n){
        REP(value, 5100){
            REP(cnt, i + 1){
                dp[value][i + 1][cnt] += dp[value][i][cnt];
                dp[value][i + 1][cnt] %= mod;
                if(value+dat[i] > 5100) continue;
                dp[value + dat[i]][i + 1][cnt + 1] += dp[value][i][cnt];
                dp[value + dat[i]][i + 1][cnt + 1] %= mod;
            }
        }
    }
    ...
    FOR(i, 0, n){
        if(dp[total / 2][n][i] != 0){
            tmp =  ((myfact(i) * myfact(n-i)) % mod) * dp[total / 2][n][i];
            res += tmp;
            res %= mod;
        }
    }
